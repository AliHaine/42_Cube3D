...
void	world_loader(t_core *core)
{
	...
	world_creator(get_world(WORLD_NETHER), core->sounds.ambiant, 32, 32, 0, core->imgs.skybox_nether, get_block_image(NETHERRACK), HARD, false, true);
	set_world_biomes(WORLD_NETHER, world_get_biomes(1, get_biome(BIOME_DARK)));
    world_generator(get_world(WORLD_NETHER));

	world_creator(get_world(WORLD_BACKROOM), core->sounds.ambiant, 32, 32, 0, core->imgs.backrooms_ceil, get_block_image(BACKROOM_FLOOR), HARD, false, false);
