...
{
	if (world->bt_color[0] > 0)
	{
		mlx_put_pixel(imgs->img_3d, dda->ray, tcd->iterator++, world->bt_color[0]);
		return ;
	}
	if (tcd->iterator == 360) {
		tcd->iterator++;
...
void	columns_drawing(t_imgs *imgs, t_dda *dda, t_world *world, t_block **blocks, t_player *player)
{
	t_col_drawing	tcd;
